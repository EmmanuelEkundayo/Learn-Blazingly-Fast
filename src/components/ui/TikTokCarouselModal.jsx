...
export default function TikTokCarouselModal({ isOpen, onClose, concept, accent, initialPlatform = 'tiktok' }) {
  ...
  return (
    <div className="fixed inset-0 z-[10001] flex items-start sm:items-center justify-center p-0 sm:p-4 bg-black/85 backdrop-blur-sm overflow-y-auto">
      ...
      <motion.div
        ...
        className="relative z-10 w-full max-w-5xl bg-[#0b0e14] border-0 sm:border border-[#1e2638] rounded-none sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col min-h-screen sm:min-h-0 sm:max-h-[96vh]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header Bar */}
        <div className="flex items-center justify-between px-3 sm:px-5 py-2.5 sm:py-3 border-b border-[#1e2638] bg-[#0d121c]">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <LBFLogo className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg shrink-0" />
            <div className="min-w-0">
              <div className="flex items-center gap-1.5 flex-wrap">
                <h2 className="text-xs sm:text-sm font-bold text-white tracking-tight">
                  {platform === 'pinterest' ? 'Pinterest' : platform === 'youtube' ? 'YouTube Shorts' : 'TikTok / Reels'} Exporter
                </h2>
                <span
                  className="text-[9px] sm:text-[10px] uppercase tracking-wider font-bold px-1.5 py-0.5 rounded-full border font-mono hidden xs:inline-block"
                  style={{
                ...
                >
                  {platform === 'youtube'
                    ? '9:16 Video'
                    : platform === 'pinterest'
                    ? (pinterestMode === 'card' ? 'Pin Card' : pinterestMode === 'carousel' ? 'Carousel' : 'Video Pin')
                    : (exportMode === 'video' ? '15s Video' : 'Carousel')}
                </span>
              </div>
              <p className="text-[10px] sm:text-xs text-slate-400 truncate hidden sm:block">
                {platform === 'pinterest'
                  ? `Export ${concept.title} as a Pin Card, Carousel, or Video.`
                  : platform === 'youtube'
                  ? `Export ${concept.title} as a YouTube Short.`
                  : `Export ${concept.title} as video or carousel.`}
              </p>
            </div>
          </div>
          ...
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-white p-1.5 rounded-lg hover:bg-[#161d2d] transition-colors shrink-0"
          >
        ...
        <div className="px-3 sm:px-5 py-2 bg-[#090d16] border-b border-[#1e2638] overflow-x-auto">
          <div className="flex items-center gap-1.5 p-1 bg-[#0d121c] border border-[#1e2638] rounded-xl w-fit min-w-full sm:min-w-0">
            <button
              onClick={() => {
                setPlatform('tiktok')
              }}
              className={`flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1.5 rounded-lg text-[11px] sm:text-xs font-bold transition-all whitespace-nowrap flex-1 sm:flex-none justify-center sm:justify-start ${
                platform === 'tiktok'
        ...
        <div className="px-3 sm:px-5 py-2 sm:py-2.5 bg-[#111622] border-b border-[#1e2638]">
          <div className="flex items-center gap-2 mb-1.5">
            <span className="w-2 h-2 rounded-full animate-pulse shrink-0" style={{ backgroundColor: selectedTheme.primary }} />
            <span className="text-[10px] sm:text-[11px] font-bold text-slate-200 uppercase tracking-wider">Theme:</span>
            <span
              className="text-[10px] sm:text-xs font-bold font-mono px-2 py-0.5 rounded-full border"
              style={{
                color: selectedTheme.primary,
            ...
            {EXPORT_THEMES.map((theme) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] sm:text-xs font-semibold border transition-all whitespace-nowrap shrink-0 ${
                    isSelected
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-4 lg:p-6 flex flex-col lg:flex-row gap-4 sm:gap-6 items-center lg:items-start justify-center">

          {/* Left: 4:5 Carousel & Video Preview */}
          <div className="flex flex-col items-center gap-3">
            {/* Slide Navigation Buttons */}
            <div className="flex items-center gap-1 p-1 bg-[#111622] border border-[#1e2638] rounded-xl overflow-x-auto max-w-full scrollbar-none w-full sm:w-auto">
              {SLIDE_NAMES.map((name, i) => (
                <button
                  ...
                  className={`px-1.5 sm:px-2.5 py-1 rounded-lg text-[10px] sm:text-xs font-semibold whitespace-nowrap transition-all flex-1 sm:flex-none ${
                    activeSlide === i
            ...
            <div className="relative rounded-2xl border border-[#1e2638] shadow-2xl p-1 bg-[#0b0e14] overflow-hidden">
              ...
              {/* Preview Window (responsive 4:5 ratio, max 360px) */}
              <div id="tiktok-preview-slide" className="w-[min(calc(100vw-3rem),360px)] sm:w-[360px] aspect-[4/5] overflow-hidden rounded-xl relative bg-[#0b0e14]">
                <SlideContent
                  index={activeSlide}
                  concept={concept}
                  card={card}
            ...
            <div className="flex items-center justify-between w-full max-w-[min(calc(100vw-3rem),360px)] sm:max-w-[360px] px-1">
              <button
                onClick={togglePlayPreview}
                className="py-1.5 px-2.5 sm:px-3 rounded-lg bg-[#111622] hover:bg-[#1a2234] border border-[#1e2638] text-[11px] sm:text-xs font-semibold text-white flex items-center gap-1.5 transition-colors shadow-sm"
              >
                {isPlayingPreview ? (
                  <>
              ...
              <div className="text-[10px] sm:text-xs text-slate-400 flex items-center gap-1 sm:gap-1.5 font-mono">
                <span>{String(activeSlide + 1).padStart(2, '0')} / {String(totalSlides).padStart(2, '0')}</span>
                <span className="hidden sm:inline">•</span>
                <span className="text-blue-400 hidden sm:inline">learnblazinglyfast.tech</span>
              </div>
            </div>
          </div>
